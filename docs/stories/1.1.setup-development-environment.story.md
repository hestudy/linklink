...
1. [ ] 配置完整的Turborepo monorepo开发环境
2. [ ] 设置所有必需的依赖包和版本锁定
3. [ ] 建立本地开发环境和工作流程
4. [ ] 配置CI/CD管道和自动化测试
5. [ ] 创建开发文档和环境配置指南
6. [ ] 验证所有三个应用（web、server、extension）可以正常启动和构建

## Tasks / Subtasks

- [ ] 任务1：初始化Turborepo monorepo结构 (AC: 1)
  - [ ] 创建或更新 `turbo.json` 配置文件
  - [ ] 设置monorepo包工作区配置
  - [ ] 配置应用间的依赖关系
  - [ ] 验证monorepo结构正确性

- [ ] 任务2：配置核心依赖包和版本 (AC: 2)
  - [ ] 更新根目录 `package.json` 与所有必需依赖
  - [ ] 配置 Bun 作为包管理器
  - [ ] 设置各应用的独立 `package.json`
  - [ ] 版本锁定和依赖解析验证

- [ ] 任务3：建立本地开发环境 (AC: 3)
  - [ ] 配置环境变量和 `.env` 文件模板
  - [ ] 设置开发脚本和热重载
  - [ ] 配置数据库连接（SQLite/Turso）
  - [ ] 创建开发环境启动脚本

- [ ] 任务4：配置CI/CD管道 (AC: 4)
  - [ ] 创建 GitHub Actions 工作流配置
  - [ ] 设置自动化构建和测试流程
  - [ ] 配置代码质量检查（ESLint, Prettier, TypeScript）
  - [ ] 设置部署流程和环境管理
...
- [ ] 任务5：创建开发文档 (AC: 5)
  ...
  - [ ] 创建开发工作流程文档
  - [ ] 提供故障排除指南
  - [ ] 文档放置在 `docs/development/` 目录

- [ ] 任务6：验证应用启动和构建 (AC: 6)
  - [ ] 验证 `apps/web` React应用可以正常启动
  - [ ] 验证 `apps/server` Hono后端可以正常启动
  - [ ] 验证 `apps/extension` 浏览器扩展可以正常构建
  - [ ] 端到端构建测试
